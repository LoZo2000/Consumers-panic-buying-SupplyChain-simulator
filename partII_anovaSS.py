...
N_dc = 3                               # Number of distribution centers 
...
t_run =  150 # 150                           # number of steps (days) in a run
...
print("IN EACH RUN, THE SAFETY STOCKS OF STORES ARE SET ACCORDING TO THE ARRAY 'SS_DCs'")


SS_DCs = [0, 50, 100, 225, 300, 500]

"""Running the simulation"""
for r in range(n_runs):
    SS_DC = SS_DCs[r]
    for b in range(run_batch):
        print("\n\nRun number:", str(r*run_batch+b), "\nNumber of SS in every DC:", SS_DC)
...
print("ANOVA on the IFR varying the SS")
from scipy.stats import f_oneway
y = np.array(IFR_normal)
print(f_oneway(y[0], y[1], y[2], y[3], y[4], y[5])) #If you run the model 6 runs